...
ReturnCode rfalDpoTableWrite( const rfalDpoEntry* powerTbl, uint8_t powerTblEntries )
{
    ...
    /* Copy the data set  */
    RFAL_MEMCPY( gRfalDpo.table, (const uint8_t*)powerTbl, (powerTblEntries * RFAL_DPO_TABLE_PARAM_LEN) );
...
ReturnCode rfalDpoTableRead( rfalDpoEntry* tblBuf, uint8_t tblBufEntries, uint8_t* tableEntries )
{
    ...
    RFAL_MEMCPY( (uint8_t*)tblBuf, gRfalDpo.currentDpo, (tblBufEntries * RFAL_DPO_TABLE_PARAM_LEN) );

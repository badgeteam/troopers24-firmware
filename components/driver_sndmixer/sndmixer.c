...
static int init_source(int* chan_id, const sndmixer_source_t *srcfns, const void *data_start,
  ...
  *chan_id = ch;
  ...
  if (chunksz <= 0)
    return 0;  // failed
  ESP_LOGI(TAG, "Sndmixer: %d: malloc chunks: %d\n", ch, chunksz);
  channel[ch].source = srcfns;
  channel[ch].volume = 1.f;
...
static void handle_cmd(sndmixer_cmd_t *cmd) {
  ...
  switch(cmd->cmd) {
    case CMD_LOOP:
      ...
      break;
    case CMD_VOLUME:
      // Volume goes from 0-255
      channel[chan_id].volume = cmd->param / 255.;
      break;
    case CMD_PLAY:
      ...
      for (int ch = 0; ch < no_channels; ch++) {
        ...
        if (chan->source && !(chan->flags & CHFL_PAUSED)) {
          ...
          if (chan->flags & CHFL_STEREO) {
            s[0] += (int32_t)((double) chan->buffer[acc * 2 + 0] * chan->volume);
            s[1] += (int32_t)((double) chan->buffer[acc * 2 + 1] * chan->volume);
          } else {
            s[0] += (int32_t)((double) chan->buffer[acc] * chan->volume);
            s[1] += (int32_t)((double) chan->buffer[acc] * chan->volume);
          }
//          ESP_LOGI(TAG, "buffer=%d volume=%f result=%d", chan->buffer[acc], chan->volume, s[0]);
        }
      }
...
int sndmixer_init(int p_no_channels, int stereo, i2s_pin_config_t* pin_config) {
  no_channels = p_no_channels;
  samplerate  = 44100;
  driver_i2s_sound_start(pin_config);
